  ...
  , fDetector(Det)
{
  ...
  fBendAngleCmd = new G4UIcmdWithADouble("/opnovice2/bendAngle", this);
  fBendAngleCmd->SetGuidance("Bend angle.");
  fBendAngleCmd->AvailableForStates(G4State_PreInit, G4State_Idle);
  fBendAngleCmd->SetToBeBroadcasted(false);
...
void DetectorMessenger::SetNewValue(G4UIcommand* command, G4String newValue)
{
  ...
  else if(command == fBendRadiusCmd)
  {
    fDetector->SetBendRadius(G4UIcommand::ConvertToDouble(newValue));
  }
  else if(command == fBendAngleCmd)
  {
    fDetector->SetBendAngle(G4UIcommand::ConvertToDouble(newValue));
  }
}
