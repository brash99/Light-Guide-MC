  ...
  , fParticleGun(0)
{
  ...
  fParticleGun->SetParticlePosition(
    G4ThreeVector(0.0 * cm, 0.0 * cm, -69.0 * cm));
...
void PrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
  if(fRandomDirection)
  {
    ...
    G4double theta3 = CLHEP::twopi * G4UniformRand();
    G4double phi   = CLHEP::twopi * G4UniformRand();
    G4double random = 2. * G4UniformRand()-1;
    //G4double theta3 = CLHEP::pi*0.5;
    //G4double phi = CLHEP::pi*0.75;
    //G4double random = 0.25;
    ...
    //G4double y     = std::sin(theta3) * std::sin(phi);
    //G4double z     = std::sin(theta3) * std::cos(phi);
    //G4double z = 0.0;
